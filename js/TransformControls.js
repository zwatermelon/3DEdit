...
THREE.TransformGizmo = function () {
	...
	this.init = function () {
		...
		this.handles = new THREE.Object3D(); //圆柱对象(可加入子object)
		this.pickers = new THREE.Object3D(); //拾取对象
		this.planes = new THREE.Object3D();  //平面

		this.add(this.handles); //控制器是所有辅助具的父Object
		...
		for ( var i in this.handleGizmos ) { /*遍历所有的可拾取物体*/
			//console.log(this.handleGizmos);
			var handle = this.handleGizmos[i][0]; // 移动圆柱mesh
			handle.name = i;
			...
			if ( this.pickerGizmos && this.pickerGizmos[i] ) {
				...
				var picker = this.pickerGizmos[i][0]; //移动拾取圆柱mesh
			...
			picker.name = i; //圆柱和拾取圆柱的名字都是［"X" ,"Y", "Z"］
		...
		this.traverse(function (child) {
			...
			if (child instanceof THREE.Mesh) {
				...
				THREE.GeometryUtils.merge( tempGeometry, child ); //将场景中所有的mesh，分别添加到一个空的几何体上，并初始化旋转，位置，大小
	...
	this.highlight = function ( axis ) {
		...
		for ( var i in this.handleGizmos ) {
			...
			if ( handle.material.oldColor ) { //恢复轴的颜色和透明度
		...
		if ( this.handleGizmos[ axis ] ) {
			...
			handle.material.oldColor = handle.material.color.clone(); //存储选中轴的颜色,和透明度
...
*@p eye  Vector3
*/
THREE.TransformGizmo.prototype.update = function ( rotation, eye ) { //鼠标移动就会调用	 ，刚开始进入的时候调用了四次
	...
	for ( var i in this.children ) { //this : 小部件类， this.children : 所有小部件的父Object

		for ( var j in this.children[i].children ) {

			var object = this.children[i].children[j]; //所有的mesh
			//console.log(object);
			if ( object.name.search("E") != -1 ) { //字符串包含大写的 “E”

				object.quaternion.setFromRotationMatrix( lookAtMatrix.lookAt( eye, vec1, vec2 ) );

			} else {

				object.quaternion.setFromEuler( rotation ); //设置Euler角(移动和缩放时欧拉角为0)
...
THREE.TransformGizmoTranslate = function () {
	...
	var arrowGeometry = new THREE.CylinderGeometry( 0.005, 0.005, 1, 4, 1, false ); //箭身 (1)
	//var arrowGeometry = new THREE.Geometry()
	var mesh = new THREE.Mesh( new THREE.CylinderGeometry( 0.0, 0.05, 0.2, 12, 1, false ) ); //移动箭头(0.2)
	mesh.position.y = 0.5;
	THREE.GeometryUtils.merge( arrowGeometry, mesh ); //(给箭身(可以是空对象)添加一个子Object(mesh))
	...
	*/
	this.setActivePlane = function ( axis, eye ) {
		...
		eye.applyProjection( tempMatrix.getInverse( tempMatrix.extractRotation( this.planes[ "XY" ].matrixWorld ) ) ); //好像没什么影响?
		//七个可移动的点
		if ( axis == "X" ) {
...
THREE.TransformGizmoRotate = function () {
	...
	this.update = function ( rotation, eye2 ) {
		...
		THREE.TransformGizmo.prototype.update.apply( this, arguments ); //当this.update()执行的时候，同时将参数传给TransformGizmo的方法update()
...
THREE.TransformGizmoScale = function () {
	...
	var arrowGeometry = new THREE.CylinderGeometry( 0.005, 0.005, 1, 4, 1, false );  //身体
	var mesh = new THREE.Mesh( new THREE.CubeGeometry( 0.125, 0.125, 0.125 ) ); //头
	mesh.position.y = 0.5;
	THREE.GeometryUtils.merge( arrowGeometry, mesh ); //合为一体(包含了mesh)
	//四个可缩放点
	this.handleGizmos = {
...
THREE.TransformControls = function ( camera, domElement ) {
	...
	this.update = function () {
		...
		worldPosition.getPositionFromMatrix( scope.object.matrixWorld ); //得到物体的世界坐标
	...
	function onPointerHover( event ) {
		...
		var intersect = intersectObjects( pointer, scope.gizmo[_mode].pickers.children ); //与场景中的激活小部件的拾取物体的所有子对象相碰撞
	...
	function onPointerDown( event ) {
		...
		if ( pointer.button === 0 || pointer.button == undefined ) {
			...
			var intersect = intersectObjects( pointer, scope.gizmo[_mode].pickers.children ); //射线与拾取Object碰撞
			...
			if ( intersect ) {
				...
				eye.copy( camPosition ).sub( worldPosition ).normalize(); //视角
				//console.log(worldPosition);
				//console.log(camPosition);
				scope.gizmo[_mode].setActivePlane( scope.axis, eye );

				var planeIntersect = intersectObjects( pointer, [scope.gizmo[_mode].activePlane] ); //射线与平面碰撞
				...
				if ( planeIntersect ) {
					...
					offset.copy( planeIntersect.point ); //存储初始碰撞点
		...
		if ( planeIntersect ) {
			...
			if ( _mode == "translate" ) {
				...
				if ( scope.space == "world" || scope.axis.search("XYZ") != -1 ) {
					console.log("move");
					if ( scope.axis.search("X") == -1 ) point.x = 0; //如果没有碰到X轴，则碰撞点的x方向为0(即移动Y或者Z,X永远为0)
					if ( scope.axis.search("Y") == -1 ) point.y = 0; //(即移动X或者Z,Y永远为0)
					if ( scope.axis.search("Z") == -1 ) point.z = 0; //(即移动X或者Y,Z永远为0)

					point.applyMatrix4( tempMatrix.getInverse( parentRotationMatrix ) ); //?

					scope.object.position.copy( oldPosition );
					scope.object.position.add( point );  //移动到碰撞点
	...
	function intersectObjects( pointer, objects ) {
		...
		ray.set( camPosition, pointerVector.sub( camPosition ).normalize() ); //反射点，发射向量

		var intersections = ray.intersectObjects( objects, true ); //返回碰撞集合
